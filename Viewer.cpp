...
#include <tikya\tutorial\sandBox\Game.h>
...
namespace glfw
{
  ...
  IGL_INLINE void Viewer::init_simplefication_objs() {
	...
	IGL_INLINE void Viewer::animate_collision() {
		...
		bool collision = detect_collision(this->trees.at(0), this->trees.at(1));
		if (collision) {
	...
	IGL_INLINE bool Viewer::detect_collision(igl::AABB<Eigen::MatrixXd, 3> *first, igl::AABB<Eigen::MatrixXd, 3> *second) {
		...
		if ( !(first->is_leaf() || second->is_leaf()) ) {
			if (!can_seperate(first->m_box, second->m_box)) {
				...
				if (!collision && !can_seperate(first_right->m_box, second_left->m_box)) {
					...
					collision = detect_collision(first_right, second_left);
				}
				if (!collision && !can_seperate(first_right->m_box, second_right->m_box)) {
					this->last_box.at(0) = &first_right->m_box;
					this->last_box.at(1) = &second_right->m_box;
					/*this->trees.at(0) = first_right;
					this->trees.at(1) = second_right;*/
					collision = detect_collision(first_right, second_right);
				}
				if (!collision && !can_seperate(first_left->m_box, second_left->m_box)) {
					this->last_box.at(0) = &first_left->m_box;
					this->last_box.at(1) = &second_left->m_box;
					/*this->trees.at(0) = first_left;
					this->trees.at(1) = second_left;*/
					collision = detect_collision(first_left, second_left);
				}
				if (!collision && !can_seperate(first_left->m_box, second_right->m_box)) {
					this->last_box.at(0) = &first_left->m_box;
					this->last_box.at(1) = &second_right->m_box;
					/*this->trees.at(0) = first_left;
					this->trees.at(1) = second_right;*/
					collision = detect_collision(first_left, second_right);
				}
			}
		}
			...
			collision = !can_seperate(first->m_box, second->m_box);
		}
		else if (first->is_leaf() && !second->is_leaf()) {
			if (!collision && !can_seperate(first->m_box, second_left->m_box)){
				this->last_box.at(1) = &second_left->m_box;
				//this->trees.at(1) = second_left;
				collision = detect_collision(first, second_left);
			}
			if (!collision && !can_seperate(first->m_box, second_right->m_box)) {
				this->last_box.at(1) = &second_right->m_box;
				//this->trees.at(1) = second_right;
				collision = detect_collision(first, second_right);
			}
		}
		else if (!first->is_leaf() && second->is_leaf()) {
			if (!collision && !can_seperate(first_left->m_box, second->m_box)) {
				this->last_box.at(0) = &first_left->m_box;
				//this->trees.at(0) = first_left;
				collision = detect_collision(first_left, second);
			}
			if (!collision && !can_seperate(first_right->m_box, second->m_box)) {
				this->last_box.at(0) = &first_right->m_box;
				//this->trees.at(0) = first_right;
				collision = detect_collision(first_right, second);
			}
		}
		if (!collision) {
			this->trees.at(0) = first;
	...
	IGL_INLINE bool Viewer::can_seperate(Eigen::AlignedBox<double, 3> &first, Eigen::AlignedBox<double, 3> &second) {
		//centers of boxes
	...
	IGL_INLINE void Viewer::init_game() {
		...
		EnemyManager* enemy_manager = new EnemyManager(10);
		this->game = new Game(this,snake,enemy_manager);
		((Game*)this->game)->init();
	}
	//-----------------------------------------------------------------
